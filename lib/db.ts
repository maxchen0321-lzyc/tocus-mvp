...
function buildCollectionFilter(
  query: ReturnType<typeof supabaseBrowser.from>,
  anonymousId: string,
  userId: string | null
) {
  if (userId == null) {
  ...
  return query.or(`user_id.eq.${userId},anonymous_id.eq.${anonymousId}`);
}
...
export async function getCollections(
  ...
  if (!hasSupabaseConfig) {
    if (typeof window === "undefined") return { data: [], error: null, source: "local" };
  ...
  const { data, error } = await buildCollectionFilter(query, anonymousId, userId);
  if (error) {
    console.error("getCollections error", error);
  }
  return { data: (data ?? []) as CollectionRecord[], error: error?.message ?? null, source: "supabase" };
}
...
export async function addCollection(
  ...
  console.log("addCollection called", { topicId, userId, anonymousId });
  ...
  if (!hasSupabaseConfig) {
    ...
    return { data: list, error: null, source: "local" } satisfies DbResult<CollectionRecord[]>;
  }

  const { error: insertError } = await supabaseBrowser.from("collections").insert(record);
  if (insertError) {
    console.error("addCollection insert error", insertError);
    return { data: [], error: insertError.message, source: "supabase" } satisfies DbResult<
      CollectionRecord[]
    >;
  }
  ...
  const { data, error } = await buildCollectionFilter(query, anonymousId, userId);
  if (error) {
    console.error("addCollection readback error", error);
  }
  return {
    data: (data ?? []) as CollectionRecord[],
    error: error?.message ?? null,
    source: "supabase"
  } satisfies DbResult<CollectionRecord[]>;
}
...
export async function removeCollection(
  ...
  if (!hasSupabaseConfig) {
    ...
    return { data: next, error: null, source: "local" } satisfies DbResult<CollectionRecord[]>;
  }

  const query = supabaseBrowser.from("collections").delete().match({ topic_id: topicId });
  if (userId == null) {
    const { error } = await query.eq("anonymous_id", anonymousId).is("user_id", null);
    if (error) {
      console.error("removeCollection error", error);
      return { data: [], error: error.message, source: "supabase" } satisfies DbResult<
        CollectionRecord[]
      >;
    }
  } else {
    const { error } = await query.match({ user_id: userId, anonymous_id: anonymousId });
    if (error) {
      console.error("removeCollection error", error);
      return { data: [], error: error.message, source: "supabase" } satisfies DbResult<
        CollectionRecord[]
  ...
  const refreshQuery = supabaseBrowser.from("collections").select("*");
  const { data, error } = await buildCollectionFilter(refreshQuery, anonymousId, userId);
  if (error) {
    console.error("removeCollection readback error", error);
  }
  return {
    data: (data ?? []) as CollectionRecord[],
    error: error?.message ?? null,
