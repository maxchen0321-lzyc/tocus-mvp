...
export default function ReadClient() {
  ...
  const userStance = stanceParam === "supporting" ? "supporting" : "opposing";
  const displayedStance = userStance === "supporting" ? "opposing" : "supporting";
  const displayedArticleStance = displayedStance === "supporting" ? "pro" : "con";

  const topic = useMemo(() => mockTopics.find((item) => item.id === topicId), [topicId]);

  const displayedArticles = useMemo(() => {
    if (!topic) return [];
    return topic.articles.filter((item) => item.stance === displayedArticleStance);
  }, [topic, displayedArticleStance]);

  const article = useMemo(() => {
    if (!displayedArticles.length) return undefined;
    if (articleIdParam) {
      return (
        displayedArticles.find((item) => item.id === articleIdParam) ?? displayedArticles[0]
      );
    }
  ...
  const oppositeArticle = useMemo(
    () => {
      ...
      const oppositeStanceValue = displayedArticleStance === "pro" ? "con" : "pro";
  ...
  const handleNextSame = async () => {
    ...
    const nextUrl = `/read?topicId=${topicId}&stance=${userStance}&articleId=${sameStanceArticle.id}&entry=same`;
  ...
  const handleNextOpposite = async () => {
    ...
    const nextUserStance = userStance === "supporting" ? "opposing" : "supporting";
    const nextUrl = `/read?topicId=${topicId}&stance=${nextUserStance}&entry=switch`;
    console.log("Navigate to:", nextUrl);
    router.push(nextUrl);
  };
        ...
        <div className="space-y-2 text-center">
          ...
          {showDebug ? (
            <p className="text-xs text-white/40">
              userStance={userStance} · displayed={displayedStance} · topicId={topicId || "none"} ·
              foundArticle={article?.id ?? "none"} · topicsCount={mockTopics.length} ·
              articlesCount={totalArticles} · availableArticles={availableArticles} · ids=
              {availableIds || "none"}
            </p>
          ) : null}
        </div>
      ...
      <div className="space-y-2">
        ...
        {showDebug ? (
          <p className="text-[10px] text-white/40">
            userStance={userStance} · displayed={displayedStance} · topicId={topicId} ·
            foundArticle={article.id}
          </p>
        ) : null}
      </div>
